...
-- Use generated column approach: billing_flag = IF(is_billing_contact=1, student_id, NULL) then unique (tenant_id, academic_year_id, billing_flag) allows only one billing=1 per student/year

ALTER TABLE guardian_student_links ADD COLUMN IF NOT EXISTS billing_unique_key BIGINT GENERATED ALWAYS AS (IF(is_billing_contact=1, student_id, NULL)) STORED;
...
-- ALTER TABLE fee_invoices MODIFY enrolment_id BIGINT UNSIGNED NOT NULL; -- commented out for now, need data cleanup first, will be done after verifying no NULLs
